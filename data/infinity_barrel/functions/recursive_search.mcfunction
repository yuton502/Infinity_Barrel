#> infinity_barrel:recursive_search
...
execute if entity @s[distance=..6] positioned ^ ^ ^0.8 unless entity @e[type=minecraft:item_frame,tag=infinity_barrel,distance=..0.1] if block ^ ^ ^-0.5 air if score $success_entity IB_data matches ..0 run function infinity_barrel:recursive_search